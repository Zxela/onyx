"""Interactive message API tests in the full Craft k8s integration lane.
...
def test_send_message_api_runs_real_celery_turn(
    ...
    try:
        # Pin the cheap tier explicitly: with no model the Build path uses the
        # provider-type recommended default (gpt-5.5), not the seeded gpt-5-mini.
