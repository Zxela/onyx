...
def pod_exec_async(
    ...
    *,
    ...
    if body is not None:
        ...
        # Suppress curl's auto Expect:100-continue: it makes the proxy respond
        # before the (large) body is read, so raw_content is None and the 403
        # carries no JSON body. Sending the body upfront hits the len-check path.
